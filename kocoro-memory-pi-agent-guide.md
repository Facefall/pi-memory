...
索引加载、embed、向量搜索、MMR 重排——放在独立进程，UDS + NDJSON 通信。Daemon 负责 spawn、ping/pong 就绪检测、graceful shutdown。
...
- **Chunking**：入库前把长文切成 ~2000 token 块，batch embed，用 `qa_id + chunk_index` 幂等。
- **MMR**：检索时先取 K×3 候选，λ=0.7 平衡相关性与多样性，避免 5 条结果全讲同一件事。

Qdrant 云端可以内置 `mmr` 参数；本地用 sqlite-vec 时，**自己在 TypeScript 里算**即可——能力不缺，只是没有「一条 SQL 带 mmr」的便利。
...
`session_compact` 事件里只 **enqueue**；Worker 规则解析 `Memory Export` → `appendIfAbsent`——**不堵**聊天热路径。

---

## Pi 落地架构（Sidecar + sqlite-vec）
...
**2. Session compaction** — `session_before_compact` 生成 dual-purpose summary；`session_compact` 异步入队；Worker 解析 Memory Export。

**3. Consolidate** — `auto-*.md ≥ 12` **或** 距上次 GC ≥ 7 天 **或** 每日 03:00（**OR** 关系，比 Kocoro 的 AND 更积极）；LLM merge → `rewrite` → `sidecar.reindex`。

读路径只有 Preflight：**Sidecar query → 失败则 readForFallback(MEMORY.md) → 仍空则 stateless**。不接 pi-session-search 进 Fallback——搜旧 session 交给 Agent 调 `session_search` 工具。
...
| Subagent Preflight | 未区分 | **cap only**（§4.3）；跳过 episodic 查询；写路径 delta 提取 |
...
1. **platform-paths** + **MemoryStore** / Markdown 后端（append、overflow、锁）
...
- Sidecar query 缓存（`indexGeneration` 在 reindex 后失效）
...
> **MEMORY.md 是源；Sidecar 是 sqlite-vec 检索进程；Preflight 读、三条链路写；Pi compaction 一次 LLM 兼顾续聊与 Export；session 搜索交给 pi-session-search；失败静默降级，不阻断主对话。**
